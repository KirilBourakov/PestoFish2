...
PYBIND11_MODULE(cEngine, m) {
    ...
    py::class_<ArrayBoard>(m, "BoardArray").def("__getitem__", [](const ArrayBoard& self, size_t i) {
        if (i >= BOARD_SIZE) {
    ...
    py::enum_<Color>(m, "Color").value("WHITE", Color::White).value("BLACK", Color::Black);

    py::enum_<CastleType>(m, "CastleType").value("LONG", CastleType::LONG).value("SHORT", CastleType::SHORT);
