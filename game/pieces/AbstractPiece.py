        ...
        for move in moves:
            ...
            if len(move) == 3:
                ...
                possible = func(
                    ...
                    move_num=board_obj.move_counter,
                )
                if not possible[0]:
                    continue
            ...
            while (
                abs(x_walker - newx) > 1 or abs(y_walker - newy) > 1
            ) and piece.hops == False:
                ...
                if board_square_not_empty:
                    not_occupied_by_en_passent = (
                        board_obj.board[y_walker][x_walker].type
                        != globals.EN_PASSANT_FLAG
                    )
                    if not_occupied_by_en_passent:
                        legal = False

            square_contains_same_color_piece = (
                (board_obj.board[newy][newx] is not None)
                and (piece.color == board_obj.board[newy][newx].color)
                and (board_obj.board[newy][newx].type != globals.EN_PASSANT_FLAG)
            )
            ...
            castling = len(move) == 3 and (
                move[2] == globals.LONG_CASTLE_FLAG
                or move[2] == globals.SHORT_CASTLE_FLAG
            )
            if castling:
                ...
                color = (
                    globals.Color.WHITE
                    if self.color == globals.Color.BLACK
                    else globals.Color.BLACK
                )
