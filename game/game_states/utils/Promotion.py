...
class Promotion:
    def __init__(self, old_pos: tuple[int, int], new_pos: tuple[int, int]) -> None:
        ...
        self.options = [PIECE.WHITE_QUEEN, PIECE.WHITE_ROOK, PIECE.WHITE_KNIGHT, PIECE.WHITE_BISHOP] if self._is_white() else \
    ...
    def show(self) -> None:
        ...
        if (self.old_pos[0] + self.old_pos[1]) % 2 == 0:
            window.blit(assets.light_square, (self.old_pos[0] * globals.grid_size, self.old_pos[1] * globals.grid_size))
        else:
            window.blit(assets.dark_square, (self.old_pos[0] * globals.grid_size, self.old_pos[1] * globals.grid_size))
