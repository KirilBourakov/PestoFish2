...
class StartState(AbstractState):
    ...
    def enter(self, args):
        ...
        self.options = [
            globals.GAME_TYPE_PVP,
            globals.GAME_TYPE_ENGINE_BLACK,
            globals.GAME_TYPE_ENGINE_WHITE,
        ]
        self.chosen = 0
    ...
    def update(self):
        ...
        window.blit(
            render, (int((globals.appsize - x) / 2), int((globals.appsize - y) / 6))
        )
        ...
        for i, option in enumerate(self.options):
            ...
            xpos, ypos = (
                int((globals.appsize - horizontal_size) / 2),
                int((globals.appsize - virtical_size) / 6) + offset,
            )
            ...
            if i == self.chosen:
                ...
                pygame.draw.rect(
                    ...
                    (255, 0, 0),
                    pygame.Rect(
                        xpos + horizontal_size + 40, ypos + 20, 80, virtical_size - 30
                    ),
                )
        ...
        window.blit(
            render,
            (int((globals.appsize - x) / 2), int((globals.appsize - y) / 6) + offset),
        )
