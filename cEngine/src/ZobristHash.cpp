...
ZobristHash::ZobristHash(const BoardArray& board,
                         Color activeColor,
                         int castlingRights,
                         std::optional<BoardPosition> enPassantLocation)
  : rng(seed)
  , dis(std::numeric_limits<unsigned long long>::min(),
        std::numeric_limits<unsigned long long>::max())
{
  auto start = std::chrono::high_resolution_clock::now();
  using namespace Pieces;

  static std::array<Piece, 12> pieces{ WHITE_PAWN, WHITE_KNIGHT, WHITE_BISHOP,
...
void
ZobristHash::recalculate(const BoardArray& board,
                         Color activeColor,
                         int castlingRights,
                         std::optional<BoardPosition> enPassantLocation)
{
...
void
ZobristHash::flipActiveColor()
{
  value ^= blackToMove;
}

void
ZobristHash::changeCastling(const int prevCastlingRights,
                            const int newCastlingRights)
{
...
void
ZobristHash::changeEnPassantSquare(
  std::optional<BoardPosition> prevEnPassantSquare,
...
void
ZobristHash::makeMove(Move move, Pieces::Piece moved, Pieces::Piece movedTo)
{
