...
bool
Evaluator::isBetterEval(const Color color, const int currBest, const int value)
{
...
int
Evaluator::evaluate(const State& state)
{
  bool endgame = false;
 ...
 */
int
Evaluator::getSquareWiseEvalAndGamePhase(const State& state, bool& endgame)
{
  ...
  for (int y = 0; y < BOARD_SIZE; y++) {
    for (int x = 0; x < BOARD_SIZE; x++) {
      if (const Pieces::Piece currPiece = state.getBoard()[y][x];
          currPiece != Pieces::EMPTY) {
        ...
        if (Pieces::sameColor(Color::White, currPiece)) {
          whiteNormalCount += normalScore.contains(colorLessPiece)
                                ? normalScore.at(colorLessPiece) * dir
                                : 0;
        } else if (Pieces::sameColor(Color::Black, currPiece)) {
          blackNormalCount += normalScore.contains(colorLessPiece)
                                ? normalScore.at(colorLessPiece) * dir
                                : 0;
        }
      }
