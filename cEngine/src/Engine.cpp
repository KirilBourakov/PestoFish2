...
Engine::getBestMove()
{
  ...
  std::ranges::sort(
    possibleMoves, [this, &entry_out](const Move& a, const Move& b) {
      return get_move_score(
               a, this->state, entry_out.bestMove, this->globalHistory) >
             get_move_score(
                ...
                HistoryTable& history)
{
  ...
  if (currGameState == GameState::DRAW ||
      currGameState == GameState::STALEMATE) {
    return 0;
  }
  ...
  std::ranges::sort(
    possibleMoves, [state, &entry_out, &history](const Move& a, const Move& b) {
      return get_move_score(a, state, entry_out.bestMove, history) >
             get_move_score(b, state, entry_out.bestMove, history);
    });
  ...
  transPosTable.insert(state.getZobrist(),
                       bestMove,
                       max_depth - curr_depth,
                       bestEval,
                       cutoffType,
                       state.getFullMoveClock());
  return bestEval;
}

int
Engine::get_move_score(const Move& move,
                       const State& state,
                       ...
                       HistoryTable& history)
{
  ...
  if (state.getAt(move.end) != Pieces::EMPTY) {
    const int victim_value =
      orderingValue.at(Pieces::piece_type(state.getAt(move.end)));
    const int attacker_value =
      orderingValue.at(Pieces::piece_type(state.getAt(move.start)));
