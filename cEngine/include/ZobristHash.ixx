...
{
public:
  ZobristHash(const BoardArray& board,
              Color activeColor,
              int castlingRights,
  ...
  void changeEnPassantSquare(std::optional<BoardPosition> prevEnPassantSquare,
...
private:
  ...
  void recalculate(const BoardArray& board,
                   Color activeColor,
                   int castlingRights,
                   std::optional<BoardPosition> enPassantSquare);
  ...
  std::unordered_map<Pieces::Piece,
                     std::array<std::array<u64, BOARD_SIZE>, BOARD_SIZE>>
    pieceTable;
  u64 blackToMove;
  std::array<u64, 16>
    castleRights; // castling rights represented by int from ob1111 -> ob0000
