...
public:
  ...
  {
    return history[move.start.y * BOARD_SIZE + move.start.x]
                  [move.end.y * BOARD_SIZE + move.end.x];
  }

private:
  std::array<std::array<int, BOARD_SIZE * BOARD_SIZE>, BOARD_SIZE * BOARD_SIZE>
    history{};
};
...
private:
  ...
  static int get_move_score(const Move& move,
                            const State& state,
...
const std::unordered_map<PieceType, int> orderingValue{
  ...
  { PieceType::Bishop, 325 }, { PieceType::Rook, 500 },
  { PieceType::Queen, 900 },  { PieceType::King, 1300 },
};
